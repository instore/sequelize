...
module.exports = (function() {
  ...
  DAO.prototype.save = function(fields) {
    fields || (fields = Utils._.keys(this.__factory.rawAttributes))
    var self = this
    
    self.executeCallbacks('beforeSave')
    if(self.isNewRecord) { self.executeCallbacks('beforeCreate') }
    ...
    if(this.isNewRecord) {
      var query = this.QueryInterface.insert(this, this.__factory.tableName, values)
      
      query.success(function(){
        self.executeCallbacks('afterCreate')
        self.executeCallbacks('afterSave')
      })
  ...
  DAO.prototype.validate = function() {
    this.executeCallbacks('beforeValidation')
    ...
    if(Utils._.isEmpty(failures)){ this.executeCallbacks('afterValidation') }

    return (Utils._.isEmpty(failures) ? null : failures)
  }

  DAO.prototype.executeCallbacks = function(callbackType) {
    self = this

    if(Utils._.isArray(self.callbacks[callbackType])) {
      Utils._.each(self.callbacks[callbackType], function(callback) {
        self._executeCallback(callback)
      })
    }
    
    if(Utils._.isString(self.callbacks[callbackType])) {
      self._executeCallback(self.callbacks[callbackType])
    }
  }

  DAO.prototype._executeCallback = function(callback) {
    if(Utils._.isString(callback)) { 
